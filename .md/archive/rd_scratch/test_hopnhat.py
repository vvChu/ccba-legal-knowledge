...
def build_hopnhat():
    ...
    # Filter out initial headers until <a id="muc-1"></a>
    idx_muc1 = 0
...
"""
    ...
    print(f"✅ Successfully written HOPNHAT_MD: {len(full_md)} chars")
    
    # Parse clauses for clauses.json
    clauses = []
    # Match any heading or anchor with number
    clause_pattern = re.compile(r'(?:<a id="([^"]+)"></a>\s*)?(?:###|####)\s*(?:<a id="([^"]+)"[^>]*></a>\s*)?([0-9]+(?:\.[0-9]+[a-z]?)*)\s*(.*?)(?=\n(?:<a id="[^"]+"></a>\s*)?(?:###|####|\Z))', re.DOTALL)
    ...
    print(f"✅ Generated AST Clauses: {len(clauses)} clauses")
    ...
    print(f"✅ Generated QA Benchmark: {len(qa_list)} QA pairs")
