# Gói Tri Thức Quy Chuẩn / Tiêu Chuẩn OKF v2.4: TCVN 3890:2023

> [!NOTE]
> **Tài liệu:** TCVN 3890:2023 — Phòng cháy và chữa cháy - Phương tiện phòng cháy và chữa cháy cho nhà và công trình - Trang bị, bố trí
> **Cơ quan ban hành:** Bộ Khoa học và Công nghệ (Quyết định số 261/QĐ-BKHCN).
> **Hiệu lực:** 2023-02-28.
> **Mỏ neo PDF Công báo:** [tcvn_3890_2023.pdf](./sources/tcvn_3890_2023.pdf) *(SHA-256: `1c22384f4a212eeb14b60e5dd12e9ab6dcac8c778fb1cf2a1c5865a4090c5e73`)*.

---

## 📑 Danh Mục Thành Phần Gói Tri Thức (OKF v2.4 Bundle)

- [Toàn văn Tiêu chuẩn (Markdown OKF v2.4)](./tcvn_3890_2023.md) — Thân văn bản quy phạm thuần khiết nguyên văn 100%.
- [Metadata Pháp lý & Đồ thị (YAML)](./metadata.yaml) — Đặc tả thuộc tính văn bản & nguồn gốc.
- [Cây Cú Pháp Điều Khoản (AST Clauses JSON)](./clauses.json) — 107 nodes điều khoản phục vụ AI QC & RAG.
- [Bộ Đánh Giá Độ Chính Xác (QA Benchmark)](./qa_benchmark.json) — 107 cặp câu hỏi - câu trả lời đối soát.
- [Kho Module Phụ Lục Kỹ Thuật (Annexes Directory)](./annexes/) — 8 Module Phụ lục quy phạm kỹ thuật (Phụ lục A đến H).
- [Bảng Tra Cứu Số Hóa (Tables Directory)](./tables/) — 11 Bảng tra cứu số học (CSV + JSON).
- [Ngăn Kéo Nguồn Gốc (Sources Directory)](./sources/) — Tệp gốc DOCX và PDF Công báo số hóa.
