...
5. **Phân Tách Rạch Ròi 4 Ngăn Kéo Dữ Liệu (4 Specialized Compartments - ADR 0036):**
   ...
   * `templates/`: Biểu mẫu hành chính nguyên tử (Atomic Form Templates theo ADR 0021, ADR 0028). Cho phép tổ chức thư mục con phụ lục (`templates/*/*.md`); Mọi công cụ thống kê, nạp RAG và kiểm định bắt buộc áp dụng giải thuật quét đệ quy (`glob("**/templates/**/*.md")` hoặc `rglob("*.md")`) để bảo toàn 100% biểu mẫu nguyên tử. Cấm để thư mục `templates/` rỗng.
...
9. **Bảo Tồn Nguyên Văn Quy Phạm & Đối Soát Song Mã 100% (Verbatim Ground Truth Parity Invariant - ADR 0037 & ADR 0045):** Nghiêm cấm mọi hành vi tóm tắt, diễn đạt lại hoặc rút gọn thân văn bản quy phạm. Áp dụng cơ chế kiểm định Verbatim Parity đối xứng song mã: ưu tiên trích xuất xác định 1:1 từ DOCX Công báo gốc; trong trường hợp văn bản chỉ có Vector PDF, đối soát trực tiếp qua giải thuật Block Multi-span Coverage (tỷ lệ bao phủ lũy tiến $\ge 70\%$ trên từng đoạn văn). Tỷ lệ khớp toàn văn bắt buộc đạt Gate 11 Verbatim Parity $\ge 98.0\%$. Đối với các văn bản có phụ lục định mức hoặc biểu mẫu quy mô lớn đã được bóc tách độc lập sang `annexes/`, `tables/` hoặc `templates/`, cho phép khai báo phạm vi thân quy phạm thuần khiết `verification_scope.normative_body_pages` trong `metadata.yaml` với điều kiện bắt buộc phải thỏa mãn Quy tắc Đối soát Bù trừ (Anti-Vacuous Pass Invariant).
...
14. **Chuẩn Hóa Cấu Trúc OpenXML DOM & Động Cơ Lai Ghép DOCX-PDF Hai Tầng (Universal Canonical OpenXML Sanitization & Hybrid Dual-Engine Invariant - ADR 0042):** Tiền xử lý 100% in-memory qua `DocxCanonicalSanitizer` (gọt `w:rsid*`, loại bỏ `w:proofErr`, gộp run liền kề đồng nhất, chuẩn hóa Unicode NFC, tiêm `xml:space="preserve"`, giải nén borderless layout tables qua Động cơ Đa nhân tố Multi-Factor Scoring Engine dựa trên Mật độ số liệu $\ge 30\%$ và Vị trí biên tài liệu, thăng cấp heading); bảo tồn tuyệt đối whitelist `<w:object>`, `<m:oMath>`, `<w:drawing>` và bảo toàn $100\%$ bảng dữ liệu theo Nguyên lý Bảo toàn Dữ liệu (Zero-Loss Conservative Invariant); kết hợp DOCX làm khung xương AST phân cấp và PDF làm mỏ neo không gian kiểm chuẩn.
15. **Lưu Trữ Song Song & Bảo Tồn Nguồn Gốc PDF (Dual-PDF Archive & Provenance Invariant - ADR 0043):** Đối với các văn bản mà tệp PDF từ TVPL/Công báo là bản scan mờ hoặc lỗi font mã hóa chữ TCVN3, Spoke bắt buộc lưu trữ bản scan gốc dưới tên `sources/<doc_slug>_raw_scan.pdf`. Đồng thời, bản Vector PDF độ nét tuyệt đối (zero-OCR) được xuất từ file DOCX chính quy qua Word COM sẽ được lưu làm `sources/<doc_slug>.pdf` và ghi nhận cờ `pdf_origin: docx_vector_rendered` trong `metadata.yaml`. Cả 2 tệp đều được bảo vệ bởi `.gitignore` và đồng bộ lên Google Drive Vault.
16. **Định Danh Bảng Quy Chuẩn Đa Phần & Cú Pháp Toán Học Đa Dòng KaTeX (Multi-Part Disambiguation & KaTeX Syntax Hierarchy Invariant - ADR 0044):** Đối với văn bản có nhiều phần độc lập (như QCVN 07:2023/BXD), bảng biểu bắt buộc phải mang tiền tố mã định danh của từng phần (ví dụ: `bang_p01_01.csv`, `bang_07_1_01.csv`) và khai báo trường `part_id` trong `tables_catalog.json`, nghiêm cấm việc ghi đè tên tệp; Đối với công thức toán học, môi trường đa dòng (`aligned`, `cases`, `gather`) bắt buộc phải sử dụng `\qquad (X)` ở cuối dòng thay cho `\tag{...}` nhằm đảm bảo không sinh lỗi bôi đỏ khi hiển thị.
17. **Quy Chuẩn Đường Dẫn POSIX Toàn Cầu & Bất Biến Đa Nền Tảng (Universal POSIX Path & Cross-Platform Invariant - ADR 0035, ADR 0036):** 100% đường dẫn khai báo trong `legal_registry.yaml` và bundle `metadata.yaml` (`bundle_path`, `pdf_path`, `raw_scan_pdf`, `source_file`, `vault_path`) bắt buộc sử dụng dấu gạch chéo thuận POSIX `/`, nghiêm cấm tuyệt đối rò rỉ ký tự Windows backslash `\`; Toàn bộ pipeline nạp văn bản (`spoke_cli.py ingest`) bắt buộc tự động khử `\`; Gate 1 và Gate 2 cưỡng chế tự động chặn lỗi định dạng đường dẫn nhằm bảo đảm khả năng thực thi đồng nhất trên Linux CI, Windows và macOS.
...
```
*Tự động thực thi toàn bộ 15 Cổng kiểm định tuần tự (Registry, OKF Bundles, Table Attachments, Fake Data, PDF Metadata, Pure Body, Cleanliness, Atomic Templates, Visual Parity, Self-Healing ADR Traceability, DOCX-to-Markdown Verbatim Parity, Multimodal Decoupled Asset & SVG/Cards Integrity, Table Knowledge 2D Regularity, KaTeX Math Syntax Integrity, và OKF Provenance & Algorithm Version Attestation).*
*Tiêu chuẩn nghiệm thu:* `0 Errors, 0 Warnings, 100% Visual Parity, 100% Verbatim Match, 100% Valid Links, 100% PDF SHA-256 Match, 100% SVG/Cards Integrity, 100% 2D Regularity, 100% KaTeX Math Integrity, 100% OKF Provenance Attestation`.
